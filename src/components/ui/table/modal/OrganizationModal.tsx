...
const OrganizationModal: React.FC<OrganizationModalProps> = ({
    ...
    const isFormValid = () => {
        ...
        if (coordinatesInputMode === "manual") {
            isCoordinatesValid =
                validateCoordinatesX(x) === "" &&
                validateCoordinatesY(y) === "";
        } else {
        ...
        if (mode === "create") {
            try {
                ...
                if (result.success) {
                    ...
                    } else if (organizationResult.status === 409) {
                        error(
                            "Creation failed",
                            `Organization with full name '${fullName}' already exists.`
                        );
                    } else {
                        error(
                            "Creation failed",
            ...
            try {
                ...
                if (result.success) {
                    ...
                    if (
                        parseInt(annualTurnover) !== organization.annualTurnover
                    )
                        updatePayload.annualTurnover = parseInt(annualTurnover);
                    if (
                        parseInt(employeesCount) !== organization.employeesCount
                    )
                    ...
                    if (
                        ...
                        result.ids.postalAddressId !==
                            organization.postalAddress.id
                    ) {
                        updatePayload.postalAddressId =
                            result.ids.postalAddressId;
                    }
                ...
                error(
                    "Update failed",
                    "Failed to update organization resources"
                );
            }
        }
    };
    ...
    const isSubmitDisabled =
        !isFormValid() || (mode === "edit" && !organization);
